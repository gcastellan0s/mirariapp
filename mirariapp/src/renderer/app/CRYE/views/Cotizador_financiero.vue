    ...
    <div class="row">
      <div class="col-sm-6 col-xl-6">
        <b-card header="INFORMACIÓN" header-tag="h6" class="mb-4">
          <div class="demo-vertical-spacing-sm">
            <b-form-group label="Valor actual del bien">
              <b-input-group prepend="$">
                <b-input type="number" placeholder="Valor actual del bien" v-model="cotizador.actual"/>
              </b-input-group>
            </b-form-group>
            <b-form-group label="Valor final del bien">
              <b-input-group prepend="$">
                <b-input type="number" placeholder="Valor final del bien" v-model="cotizador.final"/>
              </b-input-group>
            </b-form-group>
          </div>
        </b-card>
      ...
      <div class="col-sm-6 col-xl-6">
        <b-card header="VALORES" header-tag="h6" class="mb-4">
          <div class="demo-vertical-spacing-sm">
            <b-form-group label="Tasa de interés">
              <b-input-group prepend="%">
                <b-input type="number" placeholder="Expresada en términos mensuales" v-model="cotizador.interes"/>
              </b-input-group>
            </b-form-group>
            <b-form-group label="Número de periodos o rentas">
              <NumberInputSpinner :min="1" :max="100" inputClass="form-control" buttonClass="btn btn-secondary" :integerOnly="true" v-model="cotizador.plazo" />
            </b-form-group>
          </div>
        </b-card>
      </div>
        ...
        <b-card no-body class="mb-4">
          ...
          <b-card-body>
            <div class="rounded ui-bordered p-3 mb-3">
              ...
              <div class="row p-3">
                <div class="col-sm-4 col-xl-4">
                  VALOR ACTUAL DEL BIEN: <strong>{{cotizador.actual | currency | ifnot }}</strong>
                  <br />
                  VALOR FINAL DEL BIEN: <strong>{{cotizador.final | currency | ifnot }}</strong>
                  <br />
                  TASA DE INTERÉS: <strong>{{cotizador.interes | currency | ifnot }}</strong>
                  <br />
                  NÚMERO DE PERIODOS O RENTAS: <strong>{{cotizador.plazo | ifnot}}</strong>
                  <br />
                  RENTA MENSUAL: <strong>{{renta_mensual() | currency | ifnot}}</strong>
                </div>
              </div>
            </div>
...
<style src="@/vendor/libs/vue-number-input-spinner/vue-number-input-spinner.scss" lang="scss"></style>
<script>
...
export default {
  ...
  methods: {
    ...
    renta_mensual: function () {
        var n = this.cotizador.plazo
        var v = this.cotizador.actual
        var f = this.cotizador.final
        var i = this.cotizador.interes
      return (((1+i)^n)*(v*i)-(f*i))/(((1+i)^n)-1)
    },
