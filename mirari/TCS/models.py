...
VARS = {
    ...
    'LISTLENGTH': 10,
    'SEARCH': ['serial', 'client_name', 'email', 'icon_ics', 'icon_ics_2', 'icon_ics_3', 'serial_number', 'icon_os', 'icon_on', 'icon_cn', 'contact_phone1','contact_phone2','contact_phone3'],
    'SELECTQ': {
        'technical': {
            'model': ['mirari', 'User'],
            'plugin': 'select2',
    ...
    'FORM': [
        ...
        Div(
            ...
            Div('contact_phone3', css_class="col-md-4"),
...
class OrderService(Model_base):
    ...
    def QUERY(self, view):
        ...
        if len(view.request.GET.get('search[value]', '')) > 3:
            return orderService
        return orderService.filter(service_date__gt=datetime.datetime.now()-timedelta(days=120))
    ...
    def get_creation_date_html(self):
        return """
            ...
            {0}<br />
            """.format(self.creation_date.astimezone(tz.gettz('America/Mexico_City')).strftime('%d/%m/%Y %I:%M %p'))
