...
class OrderService(Model_base):
    ...
    storeName = models.CharField(max_length=250, blank=True)
    brand = models.ForeignKey('Brand', on_delete=models.SET_NULL, null=True, verbose_name="Marca")
    ...
    def QUERY(self, view):
        ...
        if len(view.request.GET.get('search[value]', '')) > 2:
            return orderService
        return orderService.filter(service_date__gt=datetime.datetime.now()-timedelta(days=60))
