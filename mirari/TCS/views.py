    ...
    for row in csv_reader: 
        ...
        if not orderService:
            try:
                ...
                orderService.creation_date = datetime.datetime.strptime(row[2].split('+')[0], '%Y-%m-%d %H:%M:%S.%f')
                ...
                orderService.buy_date = datetime.datetime.strptime(row[8], '%Y-%m-%d')
                orderService.delivery_date = datetime.datetime.strptime(row[9], '%Y-%m-%d')
                orderService.client_name = row[10]
                orderService.email = row[11]
                orderService.contact_phone1 = row[12]
                orderService.contact_phone2 = row[13]
