...
class Sellpoint__ApiView(Generic__ApiView):
	...
	def get_serializers(self, request):
		...
		if Action == 'getOnlineStatus':
			...
			cuts = Cut.objects.filter(sellpoint__organization__code=request.POST.get('organization'), id__range=(request.POST.get('init'), request.POST.get('final'))).exclude(sellpoint__id__in=[6,4,2,5]).order_by('sellpoint','id')
