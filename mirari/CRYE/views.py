	...
	def dispatch(self, request, *args, **kwargs):
		...
		if request.method == 'POST':
			if request.GET.get('api') == 'getTablaAmortizacion':
				...
				query = "select ACC.X_ORIGINAL_IMPORTE_SIN_IVA MONTO_ORIGINAL, ACC.X_ORIGINAL_NUMERO_AMORTIZACION as NUMERO_AMORTIZACIONES, AX.X_ATTRIB_95 as FECHA_LIBERACION, AX.ATTRIB_07 as ROWAM from S_FN_ACCNT1_FNX ACC, S_ASSET_X AX where ACC.par_row_id='{0}' and ACC.PAR_ROW_ID=AX.PAR_ROW_ID".format(response1[2])
				cursor = con.cursor()
				cursor.execute(query)
