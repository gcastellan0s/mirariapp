            ...
            if request.method == 'POST':
                ...
                if request.POST.get('type') == 'CREDITO':
                    ...
                    for client in response1:
                        walletCredit = WalletCredit.objects.filter(solicitud = client[1], organization = self.request.session.get('organization')).first()
                        if not walletCredit:
                            walletCredit = WalletCredit()
                        walletCredit.organization = Organization.objects.get(id=self.request.session.get('organization'))
                        walletCredit.walletcredit_tipo = 'ARRENDAMIENTO'
                        walletCredit.id_solicitud = client[0]
                        walletCredit.solicitud = client[1]
                        walletCredit.id_cliente = client[2]
                        walletCredit.tipo = client[3]
                        walletCredit.nombre = client[4]
                        walletCredit.producto = client[7]
                        walletCredit.limite_credito = client[9]
                        walletCredit.interes_ordinario = client[10]
                        walletCredit.tipo_plazo = client[13]
                        walletCredit.plazo = client[14]
                        walletCredit.save()
                        #query = "select ID_CREDITO from java_core.creditos where creditos.id_solicitud='{0}'".format(request.POST.get('unblock_number'))
                        #query = "select * from java_core.creditos where creditos.id_solicitud='{0}'".format(client[0])
                        #cursor = con.cursor()
                        #cursor.execute(query)
                        #response2 = cursor.fetchall()
                        #if len(response2) >= 1:
                            #walletCredit = WalletCredit.objects.filter(obligacion = response2[0], organization = self.request.session.get('organization')).first()
                            #if not walletCredit:
                                #walletCredit = WalletCredit()
                            #walletCredit.organization = Organization.objects.get(id=self.request.session.get('organization'))
                            #walletCredit.obligacion = request.POST.get('id')
