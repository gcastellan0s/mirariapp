...
VARS = {
    ...
    'FORM': [
        Div(
            ...
            Div(
                HTML("""<button onclick="var message = { action: 'close' }; webkit.messageHandlers.cordova_iab.postMessage(JSON.stringify(message));" class="btn btn-brand"><i class="fas fa-barcode"></i> CAPTURAR</button>"""),
                css_class="col-md-2"
            ),
            css_class="form-group m-form__group row mt-3"
        ),
